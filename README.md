...
tests/
  ...
  CASE_CATALOG.json             # GENERATED case catalog (do NOT hand-edit; see tools/gen_case_catalog.py)
  ...
  test_gen_case_catalog.py      # unit tests for tools/gen_case_catalog.py + catalog drift (no stack)
...
tools/gen_case_catalog.py       # generates tests/CASE_CATALOG.json from qa_case markers + resolver KNOWN
...
.github/workflows/unit.yml                 # PR unit lane: resolver + catalog tests + CASE_CATALOG drift check
docs/integration-tests.md       # full reference (migrated; see its banner)
```

## CASE catalog

The machine-readable source of truth is **`tests/CASE_CATALOG.json`** — generated by
`tools/gen_case_catalog.py` from the `@pytest.mark.qa_case` markers + the resolver
`KNOWN` map, and drift-checked on every PR by `.github/workflows/unit.yml`. It is what
the agentic dev-loop's QA + engineering personas read (one entry per case:
`intent / lane / mode / repo / gating / test`). The list below is a human summary.
